...
class Page(PageTreeManager, GraphicsRoundRectItem):
    ...
    def getExportFilename(self):
        return str(os.path.join(config.finalImageCachePath(), "Page_%d.png" % self._number))
    
    def getGLImageFilename(self):
        return str(os.path.join(config.glImageCachePath(), "Page_%d.png" % self._number))
    ...
    def addBlankStep(self):
        return self.insertStep(Step(self, self.getNextStepNumber()))
    ...
    def isBlank(self):
        ...
        if not result:
            for step in self.steps:
                result = step.csi.parts.__len__() == 0
                if result:
                    break
        return result
...
class LockIcon(QGraphicsPixmapItem):
    ...
    def __init__(self, parent):
        ...
        if not LockIcon.loaded:
            LockIcon.activeOpenIcon = QIcon(":/lock_open").pixmap(QSize(self._iconsize, self._iconsize)).scaledToHeight(self._iconsize)
            LockIcon.activeCloseIcon = QIcon(":/lock_close").pixmap(QSize(self._iconsize, self._iconsize)).scaledToHeight(self._iconsize)
            LockIcon.deactiveOpenIcon = QIcon(":/lock_grey_open").pixmap(QSize(self._iconsize, self._iconsize)).scaledToHeight(self._iconsize)
            LockIcon.deactiveCloseIcon = QIcon(":/lock_grey_close").pixmap(QSize(self._iconsize, self._iconsize)).scaledToHeight(self._iconsize)
