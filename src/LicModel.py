...
class PLIItem(PLIItemTreeManager, RotateScaleSignalItem, QGraphicsRectItem):
    ...
    def contextMenuEvent(self, event):
        ...
        if self.parentItem().__class__.__name__.lower()=="partlistpli": 
            menu.addAction("Put in line" ,self.alignMyGroup)
...
class CSI(CSITreeManager, RotateScaleSignalItem, QGraphicsRectItem):
    ...
    def addNewPartSignal(self):
        ...
        filepath = self.getPage().instructions.partImportDirectory
        filename = unicode(QFileDialog.getOpenFileName(self.scene().activeWindow(), "Open Part", filepath, formats))
        fn = os.path.basename(filename)
        if not fn:
            self.scene().lockApp(False)
            return
        else: 
            self.getPage().instructions.partImportDirectory = os.path.dirname(filename)
...
class Part(PartTreeManager, QGraphicsRectItem):
    ...
    def acceptColor(self, oldColor):
        ...
        for part in scene._selected:
            scene.undoStack.push( ChangePartColorCommand(part, part.color, self.color) )
        scene.restoreSelection()
    ...
    def changeBasePartSignal(self):
        loc = os.path.dirname(self.filename) if self.filename is not None else "."
        formats = LicImporters.getFileTypesString()
