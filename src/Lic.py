...
from LicHelpers import writeLogEntry, getWeightsFile,\
     BASEPLATES_FILE, SUBWINDOW_CALCULATING_TEXT
...
def __recompileResources():
    ...
    pyrcc_path = r"G:\Python27\Lib\site-packages\PyQt4\pyrcc4.exe"
...
class LicWindow(QMainWindow):
    ...
    def aboutDialog(self):
        dialog = MessageDlg(self ,QSize(455, 100))
        
        _copyright = QLabel("LEGO Instruction Creator %s\nCopyright 2015 Jeremy Czajkowski & Remi Gagne\n" % __version__)
        ...
        dialog.centreLayout.addLayout(grid)
        dialog.show()
    ...
    def loadSettings(self):
        ...
        for file in self.excludedUserParts:
            self.excludedParts.append(file)
    ...
    def fileClose(self, offerSave=True):
        ...
        if self.scene.assist is not None:
            self.scene.assist.close()
        return True
...
def setupExceptionLogger():
    ...
    logging.basicConfig(filename=os.path.join(config.appDataPath(),"licreator.log"), level=logging.DEBUG, format=f)
